...
### Phase 3B verification 🟡 (in progress)

1. ~~Pull the new worker files on the Framestation, reinstall deps, restart the worker~~ ✅
2. ~~Confirm `llama-server` is still up on `:8081`~~ ✅
3. ~~Phase 3A regression test (echo via literal mode)~~ ✅ still passes
4. ~~Phase 3B tier 1 smoke test (asyncio docs URL)~~ ✅ passes end-to-end, 4596 tokens, 3 iter
5. **Phase 3B tier 2 re-run** — rerun the canonical Nvidia lead-research task against the hardened worker (null-byte fix + content-type guard + 16384 ctx) and watch it reach `write_brief` without a single `UntranslatableCharacterError`
6. **Inspect the written brief** — open `worker/artifacts/*nvidia*.md`, eyeball quality, confirm the `artifacts` row points at the right path with `kind='report'`
7. **Long-run stability** — queue 2-3 different research tasks back-to-back to make sure the worker doesn't leak state between runs (asyncpg pool, contextvar token resets, agent iteration counters)
...
*Last updated: 2026-04-10. Project state: **Phase 3B hardened** — the Python worker has a full LLM agent loop against llama.cpp on `:8081`, four real tools (`echo`, `web_search`, `fetch_url`, `write_brief`), ContextVar-based run-id propagation, token accounting across agent iterations, tool-error feedback to the model, and input-shape dispatch preserving the Phase 3A literal-mode regression path. Phase 3A regression and Phase 3B tier-1 smoke test are both green. Tier-2 (canonical Nvidia lead-research) exposed a context-window overflow and a PostgreSQL-JSONB-null-byte bug — both now fixed: `fetch_url.py` caps at 6 K chars, rejects non-text content types, and sanitizes `\x00`; `llm.py` captures error bodies; `agent.py` logs per-iteration context size; llama-server baseline moved to `--ctx-size 16384 --n-gpu-layers 26` (3 GB VRAM headroom). Next: re-run tier-2 end-to-end, then Phase 3C (SSE log stream from Next.js) and Phase 4 (UI polish).*
