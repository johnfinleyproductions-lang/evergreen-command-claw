...
**2026-04-14 — Phase 5.4 Test H passed end-to-end, with a stale-branch trap**
Ran Test H against a live stack and hit the exact same gotcha documented in the 2026-04-11 Phase 5.0.1 entry: `evergreen restart` launched the worker from a repo still checked out to `main`, so the cancel code was not actually loaded even though the branch existed locally. First test run (`3f1b79e9-…`) finished too fast to cancel; second (`4e391189-…`) ran 8 iterations past the cancel UPDATE with no observation — classic "code not present" signature. `git branch --show-current` on the repo revealed `main`, not `phase-5.4-cooperative-cancel`. After `evergreen stop && git checkout phase-5.4-cooperative-cancel && evergreen start`, run `90eebff2-…` tripped cancel on iteration 2, finalized cleanly: `{"cancelled": true, "iterations": 2, "final_answer": null, "message_count": 6, "tool_calls_made": 2}` with `finished_at` set. Worker caught the DB flip on the very next iteration-top check. Reinforces the §13 gotcha — the `evergreen` CLI uses `$REPO` as-is and does nothing to verify branch; switching branches during a live session requires a full `evergreen restart` after checkout. Also validated the `UPDATE ... AND status = 'running'` guard prevents clobbering terminal states (first test's raw UPDATE flipped a succeeded run to cancelled — a reminder that the HTTP cancel route's `inArray(runs.status, ['pending','running'])` guard is load-bearing).
